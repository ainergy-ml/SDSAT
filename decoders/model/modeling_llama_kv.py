...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            # if nrof_update_past_kv is None:  # liu modified
            #     nrof_update_past_kv = key_states.shape[-2]
            # key_states = past_key_value[0].cat(key_states[..., :nrof_update_past_kv, :], dim=2)
            # value_states = past_key_value[1].cat(value_states[..., :nrof_update_past_kv, :], dim=2)
